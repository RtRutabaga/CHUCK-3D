...
DEST='/Game/Art/Props'
for name,expected in [('SM_HarborBoat',(628.1,222.4,494.2)),('SM_RopeCoil',(54.8,41,2.6))]:
    ...
    if expected and any(abs(a-e)>.6 for a,e in zip(actual,expected)):
        raise RuntimeError(f'Incorrect centimetre bounds: {name}: {actual} expected {expected}')
    unreal.EditorAssetLibrary.save_loaded_asset(asset,only_if_is_dirty=False)
    unreal.log(f'CHUCK_HARBOR_PROP {name} dimensions={actual}')
unreal.log('CHUCK_HARBOR_IMPORT_READY')
