...
DEST='/Game/Art/Props'
for name,expected in [('SM_DockBarrel',(62,62,90)),('SM_DockCrate',(60,65,60)),('SM_DockPlank',(23,180,12))]:
    ...
    if any(abs(a-e)>.6 for a,e in zip(actual,expected)):
        raise RuntimeError(f'Incorrect centimetre bounds: {name}: {actual} expected {expected}')
    unreal.EditorAssetLibrary.save_loaded_asset(asset,only_if_is_dirty=False)
    unreal.log(f'CHUCK_PROP {name} dimensions={actual}')
unreal.log('CHUCK_DOCK_IMPORT_READY')
